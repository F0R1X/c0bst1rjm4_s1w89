...
_ev_nodata_tstamp( 10 )
{
	...
	if( flag ){
		ev_timer_start(pdk.loop, &_ev_nodata_timer);
	}
...
Client::~Client()
{
	ev_io_stop(pdk.loop, &_ev_read);
	ev_io_stop(pdk.loop, &_ev_write);
	ev_timer_stop(pdk.loop, &_ev_nodata_timer);
...
void Client::destroy(Client *client, bool del_oldclient)
{
	...
	pdk.game->del_client(client, del_oldclient);
}
...
void Client::pre_destroy(Client *client, bool del_oldclient)
{
	...
	if (client->_write_q.empty()){
	  Client::destroy(client, del_oldclient);
		...
		log.debug("pre_destroy client->list_empty() is not empty \n");
		if (del_oldclient){
			client->cmd_type = 1;
		}
		else{
...
void Client::read_cb(struct ev_loop *loop, struct ev_io *w, int revents)
{
	...
	if (self->_state == PARSE_HEADER) {
		...
		if (self->body.length() == self->_header_p->length) {
			...
			log.debug("client[%d] uid[%d] recvDataStyled cmd[%d] \n", self->fd, self->uid, self->packet.val["cmd"].asInt());
			time_t begin = time(NULL);
			int ret = pdk.game->dispatch(self);
	...
	if (self->_state == PARSE_HEADER) {
		...
		if (self->_cur_head_len == sizeof(self->p_header)) {
			...
			if(cmd != CLINET_HEART_BEAT_REQ){
				log.info("client[%d] uid[%d] recvData cmd[%d]\n", self->fd, self->uid, cmd);
			}
			self->_state = PARSE_BODY;
			self->_cur_head_len = 0;
...
dealer:		self->_state = PARSE_HEADER;
			...
			time_t begin = time(NULL);
			int ret = pdk.game->dispatch2(self);
			time_t end = time(NULL);
			int total = end - begin;
			if (total >= 1) {
				log.error("slow cmd: [%d]\n", total);
...
int Client::send(const char *buf, unsigned int len)
{
	if (fd > 0) {
		if (_write_q.empty()) {
			_ev_write.data = this;
			ev_io_start(pdk.loop, &_ev_write);
		}
...
int Client::send(const std::string &res)
{
	if (client_type == 0) {
		...
		if (res.length() > 4) {
			...
			Json::Value m_val;
			if (m_reader.parse(body_str, m_val) >= 0) {
				log.debug("client[%d] uid[%d] sendDataStyled cmd[%d] \n", fd, uid, m_val["cmd"].asInt());
			}
		}
	}
...
{
	if( pdk.game->link_data_client && fd == pdk.game->link_data_client->fd )
	{
		return 0;
	}
	if( pdk.game->link_video_client && fd == pdk.game->link_video_client->fd )
	{
		return 0;
	}
	ev_timer_again(pdk.loop, &_ev_nodata_timer);
	return 0;
}
